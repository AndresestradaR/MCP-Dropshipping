"""
Servidor MCP para Dropi - v6.0 CORREGIDO
Ahora retorna JSON estructurado además de texto formateado
"""
...
async def get_dropi_wallet_history(args: dict) -> str:
    ...
    for mov in movements:
        ...
        # Solo contar si no fue contado ya en los primeros 20
        if mov not in movements[:20]:
            if value >= 0 or mov_type_str == "ENTRADA":
                total_in += abs(value)
            else:
                total_out += abs(value)
...
async def messages_endpoint(request):
    ...
    if method == "initialize":
        resp = {"jsonrpc": "2.0", "id": msg_id, "result": {"protocolVersion": "2024-11-05", "capabilities": {"tools": {}}, "serverInfo": {"name": "dropi-mcp", "version": "6.0.0"}}}
...
if __name__ == "__main__":
    ...
    print(f"🚀 Dropi MCP Server v6.0 - JSON MODE")
