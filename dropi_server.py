"""
Servidor MCP para Dropi - v6.4.0 FECHAS EXPLÍCITAS
Headers completos + JSON estructurado + Filtrado local + Descripciones mejoradas para fechas
"""
...
TOOLS = [
    ...
    },
    {
        ...
        "description": ⚠️ ENDPOINT ROTO - NO USAR. Este endpoint devuelve HTTP 500. Usa get_dropi_orders en su lugar para obtener movimientos y ganancias.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "days": {"type": "integer", "description": "NO USAR - Endpoint roto"}
            },
            "required": []
        }
    },
    {
        ...
        "description": """Obtiene órdenes de Dropi y calcula ganancias REALES. USA ESTA HERRAMIENTA para: historial de cartera, movimientos, entradas/salidas, cuánto ganó el usuario.

IMPORTANTE PARA FECHAS:
- Para "ayer" (ejemplo: 6 dic 2025): start_date="2025-12-06", end_date="2025-12-06"
- Para "hoy" (7 dic 2025): start_date="2025-12-07", end_date="2025-12-07"  
- Para "esta semana": usa days=7
- Para un mes específico: start_date="2025-11-01", end_date="2025-11-30"

SIEMPRE usa start_date y end_date para días específicos como "ayer" o "hoy".""",
        "inputSchema": {
            "type": "object",
            "properties": {
                "limit": {"type": "integer", "description": "Cantidad de resultados (default 100)"},
                "status": {"type": "string", "description": "Filtrar por estado"},
                "days": {"type": "integer", "description": "Últimos X días. NO usar para 'ayer' o 'hoy', usar start_date/end_date"},
                "start_date": {"type": "string", "description": "REQUERIDO para 'ayer'/'hoy'. Fecha YYYY-MM-DD. Ej: ayer=2025-12-06"},
                "end_date": {"type": "string", "description": "REQUERIDO para 'ayer'/'hoy'. Fecha YYYY-MM-DD. Igual a start_date para un solo día"}
            },
            "required": []
        }
    },
...
async def get_dropi_orders(args: dict) -> str:
    """Obtiene las órdenes."""
    # LOG para debug
    logger.info(f"📥 get_dropi_orders recibió args: {args}")
    ...
    if start_date and end_date:
        # Usar fechas específicas - necesitamos filtrar localmente
        filter_locally = True
        local_start = start_date
        local_end = end_date
        period_label = f"{start_date}" if start_date == end_date else f"{start_date} a {end_date}"
        logger.info(f"📅 Filtrado LOCAL activado: {local_start} a {local_end}")
        
        # Pedimos más órdenes para asegurar que incluya el rango
        # La API puede no filtrar bien, así que obtenemos últimos 60 días
        ...
        date_to = datetime.now().strftime("%Y-%m-%d")
        period_label = f"últimos {days} días"
        logger.info(f"📅 Usando days={days}, NO hay filtrado local")
    
    params = {
        "result_number": 200 if filter_locally else limit,  # Más órdenes si filtramos localmente
        "start": 0,
        "from": date_from,
        "until": date_to,
        "orderBy": "id",
    ...
    # FILTRADO LOCAL por fecha si se especificaron fechas
    if filter_locally and orders:
        original_count = len(orders)
        filtered_orders = []
        for order in orders:
            order_date = str(order.get("created_at", ""))[:10]  # "2025-12-05"
            if local_start <= order_date <= local_end:
                filtered_orders.append(order)
        orders = filtered_orders
        logger.info(f"📊 Filtrado: {original_count} órdenes → {len(orders)} órdenes (fecha: {local_start})")
    ...
    # Detectar si es un solo día (para formato detallado)
    is_single_day = filter_locally and local_start == local_end
    
    # Agrupar órdenes por tipo para el texto
    delivered_orders = []
    returned_orders = []
    pending_orders = []
    ...
    for order in orders:
        ...
        if status in ["entregado", "delivered", "completado", "completed"]:
            delivered_orders.append(order_info)
        elif status in ["devolucion", "devuelto", "returned"]:
            returned_orders.append(order_info)
        elif status not in ["cancelado", "cancelled", "rechazado"]:
            pending_orders.append(order_info)
    
    # Formato detallado para un solo día
    if is_single_day:
        text += f"💵 *ENTRADAS* (Órdenes Entregadas):\n"
        if delivered_orders:
            for o in delivered_orders[:20]:
                text += f"   ✅ Orden #{o['id']} - Q{o['profit']:,.2f}\n"
            if len(delivered_orders) > 20:
                text += f"   ... y {len(delivered_orders) - 20} más\n"
        else:
            text += f"   (Ninguna entrega este día)\n"
        
        text += f"\n💸 *SALIDAS* (Devoluciones - Q{return_cost_per_order:.0f} c/u):\n"
        if returned_orders:
            for o in returned_orders[:20]:
                text += f"   ❌ Orden #{o['id']} - Q{return_cost_per_order:.2f}\n"
            if len(returned_orders) > 20:
                text += f"   ... y {len(returned_orders) - 20} más\n"
        else:
            text += f"   (Ninguna devolución este día)\n"
        
        if pending_orders:
            text += f"\n⏳ *PENDIENTES* (En proceso):\n"
            for o in pending_orders[:10]:
                text += f"   🔄 Orden #{o['id']} - {o['status']} (Q{o['profit']:,.2f} si entrega)\n"
            if len(pending_orders) > 10:
                text += f"   ... y {len(pending_orders) - 10} más\n"
        
        text += f"\n"
    else:
        # Formato resumido para múltiples días - mostrar solo primeras 15 órdenes
        for order in orders[:15]:
            ...
            status_emoji = "✅" if "entregado" in status.lower() else "❌" if "devol" in status.lower() else "⏳"
            text += f"{status_emoji} #{order_id} | {customer[:15]} | Q{profit:,.2f} | {status}\n"
    ...
    json_data = {
        ...
        "delivered_orders": [{"id": o["id"], "profit": o["profit"]} for o in delivered_orders[:20]],
        
        "returned": returned_count,
        "return_cost_per_order": return_cost_per_order,
        "total_return_cost": round(total_return_cost, 2),
        "returned_orders": [{"id": o["id"]} for o in returned_orders[:20]],
...
async def messages_endpoint(request):
    ...
    if method == "initialize":
        resp = {"jsonrpc": "2.0", "id": msg_id, "result": {"protocolVersion": "2024-11-05", "capabilities": {"tools": {}}, "serverInfo": {"name": "dropi-mcp", "version": "6.4.0"}}}
