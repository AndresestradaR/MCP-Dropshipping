...
def parse_formatted_text(text: str) -> dict:
    ...
    Incluye patrones para Shopify, Meta y Dropi.
    MEJORADO: Regex más flexible para manejar emojis y espacios.
    """
    result = {}
    
    # Limpiar texto de emojis para facilitar el parseo
    # (los emojis pueden interferir con el regex)
    
    # =========================================================
    # SHOPIFY - Patrones específicos (más flexibles)
    # =========================================================
    
    # Pedidos totales: 41 (con o sin emoji antes)
    pedidos_match = re.search(r'Pedidos\s*totales[:\s]+(\d+)', text, re.IGNORECASE)
    if pedidos_match:
        result['total_orders'] = int(pedidos_match.group(1))
    
    # Total bruto: $9,500.00 (con o sin emoji antes)
    total_bruto_match = re.search(r'Total\s*bruto[:\s]+\$?([\d,]+\.?\d*)', text, re.IGNORECASE)
    if total_bruto_match:
        result['total_amount'] = float(total_bruto_match.group(1).replace(',', ''))
    
    # Pedidos pagados: 41
    pagados_match = re.search(r'Pedidos\s*pagados[:\s]+(\d+)', text, re.IGNORECASE)
    if pagados_match:
        result['paid_orders'] = int(pagados_match.group(1))
    
    # =========================================================
    # DROPI - Patrones específicos
    # =========================================================
    
    # Saldo/Balance: Q14,993.71
    saldo_match = re.search(r'Saldo\s*(?:Disponible)?:\s*[$Q]?([\d,]+\.?\d*)', text, re.IGNORECASE)
    if saldo_match:
        result['balance'] = float(saldo_match.group(1).replace(',', ''))
    
    # Entradas: Q5,000.00
    entradas_match = re.search(r'Entradas:\s*[$Q]?([\d,]+\.?\d*)', text, re.IGNORECASE)
    if entradas_match:
        result['total_income'] = float(entradas_match.group(1).replace(',', ''))
    
    # Salidas: Q2,000.00
    salidas_match = re.search(r'Salidas:\s*[$Q]?([\d,]+\.?\d*)', text, re.IGNORECASE)
    if salidas_match:
        result['total_expenses'] = float(salidas_match.group(1).replace(',', ''))
    
    # Valor Total: Q9,500.00 (para órdenes Dropi)
    valor_total_match = re.search(r'Valor\s+Total:\s*[$Q]?([\d,]+\.?\d*)', text, re.IGNORECASE)
    if valor_total_match:
        result['total_amount'] = float(valor_total_match.group(1).replace(',', ''))
    
    # Total: X (cantidad de órdenes, NO confundir con monto)
    # Solo capturar si dice "Total: X |" (formato de lista de órdenes)
    total_ordenes_match = re.search(r'Total:\s*(\d+)\s*\|', text, re.IGNORECASE)
    if total_ordenes_match:
        result['total_orders'] = int(total_ordenes_match.group(1))
    
    # =========================================================
    # META ADS - Patrones específicos (flexibles con emojis)
    # =========================================================
    
    # 💸 Gasto: $500.00 - buscar "Gasto" seguido de : y número
    gasto_match = re.search(r'Gasto[:\s]+\$?([\d,]+\.?\d*)', text, re.IGNORECASE)
    if gasto_match:
    ...
    # 💰 CPC: $4.50
    cpc_match = re.search(r'CPC[:\s]+\$?([\d,]+\.?\d*)', text, re.IGNORECASE)
    if cpc_match:
        result['cpc'] = float(cpc_match.group(1).replace(',', ''))
    
    # CPM: $4.50 (puede no tener emoji)
    ...
    # 🎯 Alcance: 1,000
    alcance_match = re.search(r'Alcance[:\s]+([\d,]+)', text, re.IGNORECASE)
    if alcance_match:
        result['reach'] = int(alcance_match.group(1).replace(',', ''))
...
async def get_dashboard_data(request: Request):
    ...
    try:
        ...
        try:
            # Mapeo mejorado de periodos para Meta Ads
            # Meta acepta: today, yesterday, last_7d, last_14d, last_28d, last_30d, last_month, this_month
            period = "last_7d"  # default
            
            period_label_lower = period_label.lower()
            
            if "hoy" in period_label_lower or "today" in period_label_lower:
                period = "today"
            elif "ayer" in period_label_lower or "yesterday" in period_label_lower:
                period = "yesterday"
            elif "7" in period_label:
                period = "last_7d"
            elif "14" in period_label:
                period = "last_14d"
            elif "28" in period_label:
                period = "last_28d"
            elif "30" in period_label:
                period = "last_30d"
            elif "mes pasado" in period_label_lower or "last month" in period_label_lower or "mes anterior" in period_label_lower:
                period = "last_month"
            elif "este mes" in period_label_lower or "this month" in period_label_lower:
                period = "this_month"
            elif "mes" in period_label_lower:
                period = "last_30d"
            
            logger.info(f"📢 Meta periodo: {period} (label: {period_label})")
