    ...
    try:
        ...
        try:
            ...
            if isinstance(wallet_history, dict):
                ...
                # Si viene con "total" pero sin income/expenses, asumir como ingreso
                # (típicamente wallet history de Dropi muestra pagos recibidos)
                if "total" in wallet_history and "income" not in wallet_history and "expenses" not in wallet_history:
                    ...
                    if total > 0:
                        ...
                        dropi_data["entradas"].append({
                            "fecha": "N/A",
                            "monto": total,
                            "concepto": "Historial de billetera"
                        })
                ...
                if "transactions" in wallet_history:
                    ...
                    if any(word in text.lower() for word in ["pago", "payment", "ingreso", "venta", "cobro"]):
                        ...
                        if "amount" in wallet_history or "total" in wallet_history:
                            monto = float(wallet_history.get("amount", wallet_history.get("total", 0)))
            ...
            # Orders puede venir como dict parseado o como lista
            if isinstance(orders_result, dict):
                # Si viene como dict parseado con totales
                if "total_orders" in orders_result or "total" in orders_result:
                    dropi_data["pedidos"]["total"] = int(orders_result.get("total_orders", 0))
                    dropi_data["pedidos"]["monto"] = float(orders_result.get("total", 0))
                    logger.info(f"📦 Pedidos (parseados): {dropi_data['pedidos']['total']}, Monto: ${dropi_data['pedidos']['monto']:.2f}")
                # Si viene como dict con lista de orders
                elif "orders" in orders_result:
