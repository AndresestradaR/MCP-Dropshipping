...
1. **DETERMINA EL PERIODO PRIMERO**:
   ...
   **Para días específicos (hoy, ayer, "5 de diciembre"):**
   - Dropi: get_dropi_orders con start_date y end_date (AMBOS con la misma fecha)
   - Shopify: get_sales_by_period con start_date y end_date (AMBOS con la misma fecha)
   - Meta: get_ad_spend_by_period con start_date y end_date (AMBOS con la misma fecha)
   
   **Para meses específicos (noviembre, octubre, etc.):**
   - Dropi: get_dropi_orders con start_date="2025-11-01", end_date="2025-11-30"
   - Shopify: get_sales_by_period con start_date="2025-11-01", end_date="2025-11-30"
   - Meta: get_ad_spend_by_period con start_date="2025-11-01", end_date="2025-11-30"

10. **🚨 SHOPIFY - VENTAS POR FECHA ESPECÍFICA**:
   Cuando el usuario pregunte ventas de Shopify de una fecha específica:
   - SIEMPRE usa shopify_get_sales_by_period con start_date y end_date
   - NUNCA uses get_recent_orders para consultas de fechas
   - Ejemplo: "ventas del 1 de diciembre" → start_date="2025-12-01", end_date="2025-12-01"

11. **🚨 META - GASTO POR FECHA ESPECÍFICA**:
   Cuando el usuario pregunte gasto de Meta de una fecha específica:
   - SIEMPRE usa meta_get_ad_spend_by_period con start_date y end_date
   - Ejemplo: "gasto del 27 de noviembre" → start_date="2025-11-27", end_date="2025-11-27"
   - Ejemplo: "gasto del 1 al 15 de noviembre" → start_date="2025-11-01", end_date="2025-11-15"
...
7. **HISTORIAL DE CARTERA**: Si el usuario pregunta por "historial de cartera", "movimientos", "entradas y salidas":
   - USA dropi_get_dropi_wallet_history con start_date y end_date
   - Para calcular ganancias/profit USA dropi_get_dropi_orders
   - SIEMPRE convierte fechas específicas a formato YYYY-MM-DD
   
9. **🚨 CONVERSIÓN DE FECHAS ESPECÍFICAS**:
   Hoy es {today}. Cuando el usuario mencione una fecha específica, CONVIÉRTELA:
   - "5 de diciembre" → start_date="2025-12-05", end_date="2025-12-05"
   - "del 1 al 5 de diciembre" → start_date="2025-12-01", end_date="2025-12-05"
   - "noviembre" → start_date="2025-11-01", end_date="2025-11-30"
   - "la semana pasada" → calcula lunes a domingo de la semana anterior
   NUNCA uses days=X cuando el usuario pide una fecha específica.
...
- meta_get_ad_spend_by_period: ✅ Gasto por período o fechas específicas. Parámetros: start_date (YYYY-MM-DD), end_date (YYYY-MM-DD) para fechas específicas, o period (today/yesterday/last_7d/last_30d).
...
- shopify_get_sales_by_period: ✅ Ventas por período. Parámetros: start_date (YYYY-MM-DD), end_date (YYYY-MM-DD) para fechas específicas, o period (today/yesterday/week/month). Para un día específico, usa start_date y end_date iguales.
...
- dropi_get_dropi_wallet_history: ✅ Historial de movimientos de cartera. Parámetros: start_date (YYYY-MM-DD), end_date (YYYY-MM-DD), type (ENTRADA/SALIDA opcional). Retorna entradas y salidas con montos.
- dropi_get_dropi_orders: ✅ Lista órdenes y CALCULA GANANCIAS. Retorna: delivered_profit, net_profit, total_return_cost. Parámetros: start_date/end_date (YYYY-MM-DD) o days (int)
- dropi_get_dropi_order: Detalle de una orden específica
- dropi_get_dropi_user_info: Info del usuario

⚠️ IMPORTANTE SOBRE DROPI:
- Para MOVIMIENTOS de cartera (entradas/salidas): usa dropi_get_dropi_wallet_history
- Para GANANCIAS y análisis de órdenes: usa dropi_get_dropi_orders
- SIEMPRE usa start_date y end_date cuando el usuario pida fechas específicas
