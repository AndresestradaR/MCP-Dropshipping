...
TOOLS = [
    ...
    },
    {
        ...
        "description": "Obtiene el gasto publicitario por periodo: today, yesterday, last_7d, last_30d",
        "inputSchema": {
            "type": "object",
            "properties": {
                "period": {"type": "string", "description": "Periodo: today, yesterday, last_7d, last_30d"}
            },
            "required": ["period"]
        }
    },
...
async def get_ad_spend_by_period(args: dict) -> str:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            result = f"""📊 META ADS ({period.upper()}):
💸 Gasto: ${spend:,.2f}
👀 Impresiones: {impressions:,}
👆 Clics: {clicks:,}"""
